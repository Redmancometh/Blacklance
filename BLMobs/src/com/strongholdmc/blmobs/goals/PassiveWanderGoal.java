...
public class PassiveWanderGoal extends BehaviorGoalAdapter
{
    ...
    private PassiveWanderGoal(NPC npc, int xrange)
    {
	this.npc = npc;
	this.xRange = xrange;
    }
    ...
    {
	forceFinish = false;
	npc.getTrait(blm.class).setTarget((RPGPlayer) null);
	;
	npc.getTrait(blm.class).setAggroLoc(null);
    }
    ...
    public static PassiveWanderGoal createWithNPC(NPC npc)
    {
	return createWithNPCAndRange(npc, 10);
    }

    public static PassiveWanderGoal createWithNPCAndRange(NPC npc, int xrange)
    {
	return new PassiveWanderGoal(npc, xrange);
    }
